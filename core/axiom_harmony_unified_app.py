...
def page_research():
    ...
    <p><strong>The cryptography, stated plainly for reviewers.</strong> Saved memory is protected with <strong>AES-256-GCM</strong> authenticated encryption. The key is derived from the person&rsquo;s own secret (the temporal anchor / return code) using <strong>PBKDF2-HMAC-SHA256 at 390,000 iterations</strong>, with a fresh random salt and nonce generated for every record. Because the key derives from a secret only the person holds, the stored text cannot be read without it &mdash; not by us, not by anyone with server access &mdash; and if the code is lost, the data is unrecoverable by design.</p>
    <div class="soft"><p style="margin:0;"><strong>Honest cryptographic limitation:</strong> AES-256-GCM is strong, modern, authenticated encryption, but it is <strong>not post-quantum</strong>. A fully quantum-resistant design would add a reviewed key-encapsulation mechanism (for example ML-KEM/Kyber) alongside authenticated symmetric encryption. We state this openly rather than overclaim &mdash; and note that data handled by third-party processors (see the Privacy Policy) is encrypted in transit and at rest but is not end-to-end encrypted.</p></div>
